...
\usepackage{graphicx}
\graphicspath{{./}{graphics/}{graphicsFromGardabae/}}
...
\textbf{\PS4} Next to the lightboard for the lights will be a hinge (see B in figure \ref{fig:Finalprodotype2} ) that can move the linkage system in the desired angle.
...
\begin{table}
	...
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{lll}
			...
			So people with  \\ && little light and \\ && camera settings \\ &&  experience can \\ && use lightbord. \\
			Ease of maintenance  &
			...
			There are only \\ && two function on \\ && the product, \\ && assemble  and \\ && disassemble .\\
...
The arm nr. 1, 3, and 4  shown in figure \ref{fig:Finalprodotype} need to be tested according to physical solutions. In the chapter, \ref{PS} is stated that the arm nr 3 need too let the camera be 2 meters away from writing surface and arm 4 makes the camera be 1.5 m from the ground, this need to be tested to see if the position of the camera is correct according to \FR1 in chapter \ref{FR}.

Arm nr. 1  in figure \ref{fig:Finalprodotype} needs to be tested according to \PS2 to see if the lights are in the correct place and fulfill its functional requirement. 

The folding mechanism  with \PS4 are tested to see if it will full fill \FR4


The Caster nr. 8  in figure \ref{fig:Finalprodotype} is added to the lightboard and it is moved around by one person if so the design will full fill \FR5 in chapter \ref{FR}
%How did you set up your tests?  (details on setup and execution)
...
%The castor wheels can handle load up to 500 N.

%For the FR one person can move the Lightboard Ready.
%The width is less than 0.8 m folded together, both linkage systems for lights and camera fold inside of the frame for the castor wheels and that is 0.8 m, see figure \ref{fig:fold} and \ref{fig:lightfold}
%What does the data mean?
%Discussion of the system's performance
%The camera is in the correct position from our calculation, both for height and distance, but in the test, the correct camera was missing and therefore it needs to be tested again.
...
\begin{table*}
	\resizebox{\linewidth}{!}{%
		\begin{tabular}{l|lll}
			& Functional Requirements                & Metric                        & Result  \\ \hline
			FR1 & Guide the camera to its correct place. & Image fills the frame 80-99\% & Unknown \\
			FR2 & Guide the lights to its correct place. & Can see face expression       & Unknown \\
			FR4 & Fold together                          & The width is less than 0.8 m  & Success \\
			FR5 & Move studio in one unit                & Can one person move it        & Success
		\end{tabular}%
	}
...
%but in the test, the correct camera was missing and therefore it needs to be tested again.

%The lights are stable and it is possible to change the height of them if necessary (see figure \ref{fig:LightonHannes}).
The light was tested in too much light and therefore we could not determent for sure if it's good or not.


%For a person with no experience of the Lightboard Ready, it is simply to move it without help in the correct location and takes less than 5 minutes to set it up.
%Equally easy is to fold it together and transport it to storage.
